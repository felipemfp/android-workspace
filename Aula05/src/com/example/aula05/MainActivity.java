...
public class MainActivity extends Activity {
	...
	public void btnDialog6Click(View v) {
		Calendar data = Calendar.getInstance();
		int dia = data.get(Calendar.DAY_OF_MONTH);
		int mes = data.get(Calendar.MONTH);
		int ano = data.get(Calendar.YEAR);
		DatePickerDialog dialog = new DatePickerDialog(this, listener6, ano,
				mes, dia);
		dialog.setTitle(R.string.app_name);
		dialog.show();
	}

	public void btnDialog7Click(View v) {
		Calendar data = Calendar.getInstance();
		int hor = data.get(Calendar.HOUR_OF_DAY);
		int min = data.get(Calendar.MINUTE);
		TimePickerDialog dialog = new TimePickerDialog(this, listener7, hor,
				min, true);
		dialog.setTitle(R.string.app_name);
		dialog.show();
	}

	private OnDateSetListener listener6 = new OnDateSetListener() {

		public void onDateSet(DatePicker view, int year, int monthOfYear,
				int dayOfMonth) {
			String msg = String.format("%d/%d/%d", dayOfMonth, monthOfYear,
					year);
			Toast.makeText(MainActivity.this, msg, Toast.LENGTH_SHORT).show();
		}
	};

	private OnTimeSetListener listener7 = new OnTimeSetListener() {
